...
# Let /system/bin/sh can use tools in '/system/bin'
...
if [ $(getprop persist.log.tag.aplogfiles) -gt 0 ]; then
    for svc in mainlog radiolog eventslog; do
        eval ${svc}=$(getprop persist.log.tag.aplog.${svc})
        if eval [ "\$$svc" = "true" ]; then
            stop $svc"_big"
        fi
    done
else
...
tar zcf /data/local/newlog/${FILENAME}.tgz -C ${APLOG_DIR}/../ aplog
wait
...
rm -rf $APLOG_DIR/vendor_tombstones/*
rm -rf $APLOG_DIR/wlan_logs/*
