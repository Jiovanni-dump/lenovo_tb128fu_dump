...
array_p14=(
        ...
        "health"				"/sys/class/power_supply/battery/health"
        "usb_online"			"/sys/class/power_supply/usb/online"
        "charger current"			"/sys/kernel/charger_cooler/charger_curr"
        "charger cp current"			"/sys/kernel/charger_cooler/charger_cp_curr"
        "is_cpu_limit"			"/sys/kernel/thermal/is_cpu_limit"
        "is_gpu_limit"			"/sys/kernel/thermal/is_gpu_limit"
        "is_apu_limit"			"/sys/kernel/thermal/is_apu_limit"
        "target_tpcb"			"/sys/kernel/thermal/target_tpcb"
        "cpu ttj,gpu ttj,apu ttj"				"/sys/kernel/thermal/ttj"
        "min_ttj"				"/sys/kernel/thermal/min_ttj"
        "cpu online"			"/sys/devices/system/cpu/online"
        "LL min opp hint,BL min opp hint,B min opp hint,LL limit freq,BL limit freq,B limit freq,LL cur freq,BL cur freq,B cur freq,LL max temp,BL max temp,B max temp"			"/sys/kernel/thermal/cpu_info"
		"gpu temp,gpu limit freq,gpu cur freq"			"/sys/kernel/thermal/gpu_info"
)
...
if [[ "$product" == "diablo" ]] || [[ "$product" == "diablo_fac" ]]; then
    ...
    do
        if [[ "$pro_name" == "" ]] || [ ! -f ${BATT_LOGFILE} ]; then
            # add header
            pro_name="time,version,log_cnt,rec_cnt,platform,product,"

			while [ $i -lt $len ]
			do
			echo ${array_p14[i+1]}
				if [ -r ${array_p14[i+1]} ]; then
					freq_path+="${array_p14[i+1]}"" "
					pro_name+="${array_p14[i]}",
				fi
				((i=i+2))
			done
        ...
        # add header
        pro_data="`echo $(date "+%Y-%m-%d %H:%M:%S.%3N")`,${VER},${file_count},${count},${platform},${product},"
        pro_data+=`cat ${freq_path} | tr '\n' ','`
        pro_data+=`cat ${dev_path} | tr '\n' ','`
        pro_data+=`cat ${tz_path} | tr '\n' ','`
